...
import { Pressable, ScrollView, Text, TextInput, useColorScheme, useWindowDimensions, View } from 'react-native';
...
export function AdminShell({ children }: PropsWithChildren) {
  ...
  const toggleSidebar = () => { if (compact) setDrawer(!drawer); else { const next = !collapsed; setCollapsed(next); localStorage?.setItem('sb-admin-sidebar', next ? 'collapsed' : 'expanded'); } };
  const sidebarVisible = !compact || drawer;
  return <AdminUiProvider value={{ dark, locale, setDark, setLocale }}><View style={{ flex: 1, minHeight: '100%', flexDirection: 'row', backgroundColor: bg }}>
    {sidebarVisible && <View accessibilityLabel="Primary navigation" style={{ width: compact ? 280 : collapsed ? 76 : 244, backgroundColor: '#0b1220', paddingVertical: 18, position: compact ? 'absolute' : 'relative', zIndex: 20, left: 0, top: 0, bottom: 0 }}>
      <Pressable accessibilityRole="button" accessibilityLabel={t(locale, 'toggleNavigation')} accessibilityHint={t(locale, 'toggleNavigationHint')} onPress={toggleSidebar} style={{ paddingHorizontal: 18, paddingBottom: 18 }}><Text style={{ color: '#fff', fontSize: 18, fontWeight: '800' }}>{collapsed && !compact ? 'SB' : 'Second Brain'}</Text><Text style={{ color: '#60a5fa', fontSize: 10 }}>CONTROL CENTER</Text></Pressable>
    ...
    <View style={{ flex: 1, minWidth: 0 }}>
      <View style={{ height: 66, paddingHorizontal: width < 768 ? 12 : 22, flexDirection: 'row', gap: 12, alignItems: 'center', backgroundColor: panel, borderBottomWidth: 1, borderBottomColor: dark ? '#263244' : '#e2e8f0' }}>
        ...
        <View style={{ flex: 1 }}><Text numberOfLines={1} style={{ color: fg, fontSize: 17, fontWeight: '700' }}>{t(locale, selected)}</Text><Text style={{ color: dark ? '#94a3b8' : '#64748b', fontSize: 11 }}>Admin / {t(locale, selected)}</Text></View>
        {width >= 768 && <TextInput editable={false} accessibilityLabel="Search placeholder" placeholder={t(locale, 'search')} placeholderTextColor="#94a3b8" style={{ width: 190, borderWidth: 1, borderColor: dark ? '#334155' : '#cbd5e1', borderRadius: 7, padding: 8, color: fg }} />}
        ...
        {width >= 768 && <Pressable accessibilityLabel="Notifications"><Text style={{ fontSize: 18 }}>◉</Text></Pressable>}
        <Pressable accessibilityRole="button" accessibilityLabel={dark ? t(locale, 'switchToLightTheme') : t(locale, 'switchToDarkTheme')} accessibilityHint={t(locale, 'themeHint')} onPress={() => { const next = !dark; setDark(next); localStorage?.setItem('sb-admin-theme', next ? 'dark' : 'light'); }}><Text style={{ fontSize: 18 }}>{dark ? '☀' : '☾'}</Text></Pressable>
        ...
        <Pressable accessibilityRole="button" accessibilityLabel={locale === 'en' ? t(locale, 'switchToFrench') : t(locale, 'switchToEnglish')} accessibilityHint={t(locale, 'languageHint')} onPress={() => { const next = locale === 'en' ? 'fr' : 'en'; setLocale(next); localStorage?.setItem('sb-admin-locale', next); }}><Text style={{ color: fg, fontSize: 11 }}>{locale.toUpperCase()}</Text></Pressable>
      </View>
