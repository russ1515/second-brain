...
export function SupportCenter() {
  ...
  const load = useCallback(async () => { if (!canRead) return; setLoading(true); setError(null); try { const selectedStatus = tab === 'open' ? 'open' : tab === 'resolved' ? 'resolved' : transportEnum(filters.status); setData(await getSupportCases({ page: 1, pageSize: 50, priority: transportEnum(filters.priority), search: filters.search || undefined, ...(tab === 'reports' ? { view: 'reports' } : {}), ...(tab === 'mine' ? { assignee: 'me' } : {}), status: selectedStatus })); } catch (problem) { setError(safeProblem(problem)); } finally { setLoading(false); } }, [canRead, filters, tab]);
