...
export function UserControlCenter({ userId }: { userId: string }) {
  ...
  const accountStatus = recordString(user, ['accountStatus', 'status']); const plan = recordString(user, ['plan', 'planSlug']); const subscriptionStatus = recordString(user, ['subscriptionStatus', 'subscriptionStatusCode', 'subscription']); const quotaState = recordString(user, ['quotaState', 'quotaStatus']);
  ...
  return <View style={{ gap: 16, paddingBottom: 30 }}>
    ...
    <View style={{ gap: 14, padding: width < 700 ? 15 : 20, borderWidth: 1, borderColor: theme.border, borderRadius: 14, backgroundColor: theme.surface }}>
      ...
      <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}><ActionButton locale={locale} theme={theme} label="suspendUser" allowed={capability(identity, actions.suspend.capability) && accountStatus !== 'SUSPENDED'} onPress={() => begin('suspend')} /><ActionButton locale={locale} theme={theme} label="reactivateUser" allowed={capability(identity, actions.reactivate.capability) && accountStatus === 'SUSPENDED'} onPress={() => begin('reactivate')} /><ActionButton locale={locale} theme={theme} label="banUser" allowed={capability(identity, actions.ban.capability) && accountStatus !== 'BANNED'} onPress={() => begin('ban')} danger /><ActionButton locale={locale} theme={theme} label="revokeAllSessions" allowed={capability(identity, actions['revoke-sessions'].capability)} onPress={() => begin('revoke-sessions')} /></View>
    </View>
