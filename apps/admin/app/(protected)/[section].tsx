...
export default function SectionPage() {
  ...
  const level = privacy[section as Section] ?? 'STANDARD'; const muted = dark ? '#94a3b8' : '#475569'; const restricted = dark ? '#f87171' : '#b91c1c';
  return <View style={{ gap: 16 }}><View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}><View><Text accessibilityRole="header" style={{ fontSize: 26, fontWeight: '800', color: dark ? '#f8fafc' : '#0f172a' }}>{t(locale, section as Section)}</Text><Text style={{ color: muted }}>{t(locale, 'protectedRoute')}</Text></View><Text style={{ color: level === 'HIGHLY_RESTRICTED' ? restricted : muted, fontSize: 11, fontWeight: '800' }}>{level}</Text></View>
    <View style={{ minHeight: 260, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderStyle: 'dashed', borderColor: dark ? '#475569' : '#cbd5e1', borderRadius: 12, backgroundColor: dark ? '#111827' : '#fff', padding: 28 }}><Text style={{ fontSize: 20, fontWeight: '700', color: dark ? '#e5e7eb' : '#0f172a' }}>{t(locale, 'empty')}</Text><Text style={{ color: muted, textAlign: 'center', marginTop: 8 }}>{t(locale, 'foundationDetail')}</Text></View>
  </View>;
}
