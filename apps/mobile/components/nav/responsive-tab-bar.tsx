 ...
 *  • Desktop (≥1024px): a permanent, RETRACTABLE vertical sidebar on the left.
...
function Sidebar(props: BottomTabBarProps) {
  ...
  return (
    <View
      ...
      style={{ position: 'absolute', left: 0, top: 0, bottom: 0, width: collapsed ? 68 : 232, backgroundColor: c.surface, borderRightWidth: 1, borderRightColor: c.borderSubtle, padding: spacing.sm, gap: 4, zIndex: 10 }}
    >
      <Pressable onPress={() => setCollapsed((v) => !v)} accessibilityRole="button" accessibilityLabel={collapsed ? 'Déplier le menu' : 'Replier le menu'}
        style={{ flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: spacing.sm, paddingVertical: spacing.sm }}>
      ...
      {items(props).map(({ route, active, meta, onPress }) => (
        <Pressable key={route.key} onPress={onPress} accessibilityRole="tab" accessibilityState={{ selected: active }} accessibilityLabel={t(meta.key)}
          style={{ flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 11, paddingHorizontal: spacing.sm, borderRadius: radius.sm, backgroundColor: active ? c.aiAccentSoft : 'transparent', minHeight: 44 }}>
          <Text style={{ fontSize: 18 }}>{meta.icon}</Text>
          {!collapsed ? <Text style={{ color: active ? c.aiAccent : c.textSecondary, fontSize: 15, fontWeight: active ? '700' : '500' }}>{t(meta.key)}</Text> : null}
        </Pressable>
      ))}
    </View>
  );
